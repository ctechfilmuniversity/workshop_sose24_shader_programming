...
float sdfScene(vec3 point_on_ray) 
{
    ...
    // float distance_sphere1 = sdfSphere(point_on_ray - vec3(0., + sin(u_time), 2), 0.5);
...
vec3 render(vec2 xy)
{
    ...
    vec2 p = (2.0 * xy - u_resolution.xy) / u_resolution.y;
